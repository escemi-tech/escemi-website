...
export const SECTION_LINKS: Record<
	Locale,
...
const TRANSLATIONS: Record<Locale, LocaleTranslations> = {
	fr: {
		hero: { greeting: "Bonjour, je suis", cta: "Discutons de votre project" },
		services: { title: "Expertise & Services" },
		impact: { title: "Impact Mesurable" },
		experience: {
			title: "Projects & Réalisations",
			description:
				"Parcourez une sélection de cas clients. Retournez chaque project pour révéler le playbook mission, les temps forts et un accès vers le récit complete.",
			...
			cta: "Voir mon profil complete",
		},
		contact: {
			title: "Prêt à transformer vos projects ?",
			description:
...
function createLocaleCopy(locale: Locale): LocaleCopy {
	...
	const signatureNumbers = (resume.basics?.signatureNumbers ?? []).filter(
		(item) => Boolean(item?.trim?.()),
	);
	const contactDescription = translations.contact.description;
	const contactPositions = getServices(locale)
		.map((service) => {
			const summary = service.summary?.trim() ?? "";
			const description = service.description?.trim() ?? "";
			if (!service.name || (!summary && !description)) {
				return undefined;
			}
			return {
				title: service.name,
	...
	const resumeLinks = (Object.keys(RESUME_PDFS) as Locale[]).map(
		(resumeLocale) => ({
			locale: resumeLocale,
