...
const mockedGetFeaturedProjects = vi.fn(
	(_locale?: ResumeLocale, limit?: number): ResumeProject[] => {
		const subset =
			typeof limit === "number" ? mockProjects.slice(0, limit) : mockProjects;
...
vi.mock("../../data/resumes/index", async () => {
	const actual = await vi.importActual<
		typeof import("../../data/resumes/index")
	>("../../data/resumes/index");
	return {
		...actual,
...
describe("getFeaturedProjectCards", () => {
	...
	it("formats timeframes for the available resume project shapes", () => {
		...
		expect(
			cards.map((card) => ({ name: card.name, timeframe: card.timeframe })),
		).toEqual([
