...
type ResumeProjectMerged = ResumeCommon["projects"][number] &
	ResumeLocaleData["projects"][number];
type ResumeWorkMerged = ResumeCommon["work"][number] &
	ResumeLocaleData["work"][number];
type ResumeCertificateMerged = ResumeCommon["certificates"][number] &
	ResumeLocaleData["certificates"][number];

type ResumeMerged = Omit<
	ResumeCommon,
	"basics" | "projects" | "work" | "certificates"
> &
...
function mergeArray(
	base: readonly ResumeArrayItem[] = [],
	override: readonly ResumeArrayItem[] = [],
): ResumeArrayItem[] {
	if (!base.length) {
		return [...override];
	}
	...
	const hasId = (
		item: ResumeArrayItem,
	): item is ResumeArrayItem & { id: string } => typeof item.id === "string";
...
function mergeResumeData(
	...
	return {
		...
		awards: mergeArray(
			baseLocale.awards ?? [],
			override.awards ?? [],
		) as ResumeMerged["awards"],
		certificates: mergeArray(
			base.certificates ?? [],
			override.certificates ?? [],
		) as ResumeMerged["certificates"],
		education: mergeArray(
			baseLocale.education ?? [],
			override.education ?? [],
		) as ResumeMerged["education"],
		interests: mergeArray(
			baseLocale.interests ?? [],
			override.interests ?? [],
		) as ResumeMerged["interests"],
		publications: mergeArray(
			baseLocale.publications ?? [],
			override.publications ?? [],
		) as ResumeMerged["publications"],
		references: mergeArray(
			baseLocale.references ?? [],
			override.references ?? [],
		) as ResumeMerged["references"],
		skills: mergeArray(
			baseLocale.skills ?? [],
			override.skills ?? [],
		) as ResumeMerged["skills"],
		volunteer: mergeArray(
			baseLocale.volunteer ?? [],
			override.volunteer ?? [],
		) as ResumeMerged["volunteer"],
		work: mergeArray(
			base.work ?? [],
			override.work ?? [],
		) as ResumeMerged["work"],
		projects: mergeArray(
			base.projects ?? [],
			override.projects ?? [],
		) as ResumeMerged["projects"],
	};
}
...
const profilesByLocale: Record<
	ResumeLocale,
	Record<string, ResumeProfile>
> = Object.keys(RESUMES).reduce(
	(acc, key) => {
		const locale = key as ResumeLocale;
		acc[locale] = (RESUMES[locale].basics?.profiles ?? []).reduce(
			(profiles, profile) => {
...
export function getFeaturedProjects(
	...
	const featured = (getResume(locale).projects ?? []).filter(
